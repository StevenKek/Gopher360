...
int main()
{
	...
	printf("See the GitHub repository at bit.ly/1syAhMT for more info.");

	printf("\n\n-------------------------\n\n");
	SetConsoleTextAttribute(hConsole, 5); // set color to purple on black (windows only); 29 default

	// dump important tips
	printf("Tip - Press SELECT and START simultaneously to toggle Gopher state (active/inactive)! (Configurable in config.ini)\n");
	printf("Tip - Press LS and RS simultaneously to toggle speeds! (Configurable in config.ini)\n");
	printf("Tip - Hold SELECT to switch active displays! You can also use Win+Shift+P. (Configurable in config.ini)\n");
